...
@Service
public class UserRetrievingStrategy implements RetrievingStrategy {

    private final ApplicationRepository applicationRepository;

    public UserRetrievingStrategy(ApplicationRepository applicationRepository) {
        this.applicationRepository = applicationRepository;
    }
    
    public PagingDTO<ApplicationResponseDTO> retrieveApplications(UUID id, int page, int size) {
        Specification<Application> spec = Specification.where(ApplicationSpecification.ownedByUserWithId(id))
